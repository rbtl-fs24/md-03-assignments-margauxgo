...
sanitation_afg <- sanitation %>% 
  filter(name == "Afghanistan", year %in% c(2000, 2020), varname_short != "san_sm")
```
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend? It is not intuitive
2.  What would you want to change? I'll change the order of the bars to urban, rural and national and also modify the displayed percentages to have basic sanitation services at the bottom of each bar and then as one goes up limited, no sanitation and unimproved sanitation facilities
3.  Why did we remove "safely managed sanitation services" from the data set in Task 3? Because they represent a minority and are therefore not of big interest

## Task 7

1.  Run the code in the code chunk below.
2.  What do you observe when you look at the code and plot? We recognize the same patterns for national, rural and urban areas and observe that the majority of the sanitation facilities in 2020 are basic sanitation services
